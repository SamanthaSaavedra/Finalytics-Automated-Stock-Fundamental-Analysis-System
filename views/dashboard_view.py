...
def _int_axis_labels(vmin: float, vmax: float, target: int = 6) -> list[ft.ChartAxisLabel] | None:
    """
    Create Y-axis labels with INTEGER values (1-2-5*10^k).
    If it cannot produce at least 2 ticks, return None to allow Flet's auto-tick.
    """
    if vmax <= vmin:
        return None
...
def _estimate_fundamentals_height(income, margins_by_year, balance, ratios_by_year, page: ft.Page) -> int:
    """Estimate a reasonable height so tables neither collapse nor leave huge gaps.
    It is based on how many rows (metrics) the largest dataset has.
    """
    ...
    # approximate height: height per row + headers + padding
    per_row = 44
    base   = 160
...
def _transpose_year_dict_to_table(year_dict: dict | None) -> dict:
    ...
    Input by years:
      { 2024: {"A": "x", "B": "y"}, 2023: {...}, ... }
    Output uses years as columns and metrics as rows.
    """
...
def _build_charts_panel(income_by_year: dict | None, page: ft.Page) -> ft.Control:
    """
    Responsive charts with grid and year labels:
      - Revenue (billions)
      - Net Income (billions)
    """
    ...
    H_TOTAL = max(_pane_height(page) - 150, 700)   # desired total height
    H_CHART = max(H_TOTAL // 2, 320)               # each chart height
...
def _toolbar(symbol: str, page: ft.Page):
    ...
    def toggle_watchlist(_):
        nonlocal is_fav
        (remove_from_watchlist if is_fav else add_to_watchlist)(symbol, APP_USER)
        is_fav = not is_fav
        star_btn.icon = ft.Icons.STAR if is_fav else ft.Icons.STAR_BORDER
        star_btn.icon_color = "#eab308" if is_fav else "#9aa6b2"
        page.snack_bar = ft.SnackBar(ft.Text("Watchlist updated"))
...
def _empty_state(symbol: str, page: ft.Page, title=None, body=None):
    return Card(
        ...
        ft.Text(body or (
            f"No data in the database for {symbol}. "
        ...
        [
            ...
            ft.Column(
                controls=[
                    ...
                    # First row: Country | Exchange | Ticker with icons
                    ft.Row(
                        controls=[
                    ...
                    # Second row: Sector | Industry with icons
                    ft.Row(
                        controls=[
...
def _datatable(title: str, table_data: dict, show_title: bool = True, page: ft.Page = None):
    ...
    for i, r in enumerate(table_data.get("rows", [])):
        row_color = page.bgcolor if i % 2 == 0 else ft.colors.INDIGO_500  # alternating rows
        rows.append(
            ft.DataRow(
    ...
    table = ft.DataTable(
        ...
        bgcolor=page.bgcolor if page else None  # table background
    )

    card_content = [ft.Text(title, size=16, weight="bold"), table] if show_title else [table]

    # Outer container using the page background color
    return ft.Container(
        content=ft.Card(*card_content),  # DO NOT set bgcolor on Card
        expand=True,
        padding=5,
        bgcolor=page.bgcolor if page else None  # correct background
    )
...
def _section_table_transposed(title: str, data_by_year: dict | None, symbol: str, page: ft.Page):
    ...
    if _has_table_data(tbl):
        return _datatable(title, tbl, show_title=False, page=page)  # <- here
    else:
        return _empty_state(symbol, page, f"{title} — No data")
...
def _analysis_block(symbol: str, page: ft.Page):
    ...
    rec = db["analysis"].find_one({"symbol": sym}) or {}
    current_summary = rec.get("summary")

    doc = db["docs"].find_one({"_id": sym}) or {}
    preview_txt = ((doc.get("trend_summary") or "") + "\n\n" + (doc.get("table_summary") or "")).strip()

    body_text = current_summary or (preview_txt or "No analysis. You can generate it using the button below.")
    body = ft.Text(body_text, selectable=True)

    def run_generation(_):
        if not os.getenv("SEC_API_KEYS", "").strip():
            body.value = "Set SEC_API_KEYS in your environment."
            page.update(); return
        if not os.getenv("DEEPSEEK_API_KEY", "").strip():
            body.value = "Set DEEPSEEK_API_KEY in your environment."
            page.update(); return
        ...
        async def run():
            try:
                ensure_company_doc = import_module("rag.generate_rag_documents").ensure_company_doc
                generate_summary = import_module("rag.summary_company").generate_summary
                ensure_company_doc(sym, verbose=False)
                summary = generate_summary(sym, persist=True)
                body.value = summary
                page.snack_bar = ft.SnackBar(ft.Text("Analysis generated and saved"))
                page.snack_bar.open = True
            except Exception as e:
                body.value = f"Could not generate the analysis: {e}"
            finally:
                btn.disabled = False
                btn.text = "Regenerate analysis" if current_summary else "Generate analysis"
                page.update()
        page.run_task(run)

    btn = ft.ElevatedButton(
        "Regenerate analysis" if current_summary else "Generate analysis",
        icon=ft.Icons.PLAY_CIRCLE, on_click=run_generation
    )
...
def _fundamentals_tabs(
    ...
    def _pane(content: ft.Control) -> ft.Control:
        return ft.Container(
            padding=20,
            expand=True,
            bgcolor=page.bgcolor,  # <--- page background color here
            content=content
        )
    ...
    # Wrap the tabs in a container with the page background
    return ft.Container(
        content=tabs_inner,
        expand=True,
        bgcolor=page.bgcolor
...
def _analysis_and_charts_tabs(
    ...
    return ft.Container(
        ...
        expand=True,
        bgcolor=page.bgcolor  # <-- background for the whole section
    )
...
def _swap_tab_content(e: ft.ControlEvent,
    ...
    """Callback to swap the displayed panel under the header."""
...
def build_dashboard_view(symbol: str, page: ft.Page) -> ft.View:
    ...
    # Datasets
    try: overview = data.get_overview_data()
    except: overview = None
    try: margins_by_year = data.get_margins_data()
    except: margins_by_year = None
    try: income = data.get_income_statement_data()
    except: income = None
    try: balance = data.get_balance_sheet_data()
    except: balance = None
    try: ratios_by_year = data.get_financial_ratios_data()
    except: ratios_by_year = None
