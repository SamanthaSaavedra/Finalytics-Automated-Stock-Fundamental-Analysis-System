...
class CompanyDashboard:
    def __init__(self, overview, income, margins, balance, ratios):
    ...
    def render(self):
        ...
        header = ft.Card(
            content=ft.Container(
                content=ft.Column(
                    controls=[
                        # Primera fila: logo e info principal
                        ft.Row(
                            controls=[
                                # Logo de la empresa
                                ...
                                ft.Column(
                                    controls=[
                                        ...
                                        # Información adicional: País, Intercambio y Ticket
                                        ft.Row(
                                            controls=[
        ...
        cards.append(
            ft.Text(
                "  Fundamental Analysis" + f" ({self.overview['Currency']})",
                size=20,
                weight="bold",
        ...
        for title, table_data in self.tables.items():
            column_width = 150  # Establecemos un ancho fijo para todas las columnas
            table = ft.DataTable(
                columns=[
                    # Primera columna con estilo personalizado
                    ft.DataColumn(
                        ft.Text(
                            table_data["columns"][0],
                            size=13,
                            weight="bold",
                            color=ft.Colors.BLUE_500  # Puedes elegir otro color si prefieres
                        )
                    )
                ] + [
                    # Resto de las columnas con estilo estándar
            ...
            expansion = ft.ExpansionTile(
                title=ft.Text(f"   "+title, size=18, weight="bold", color=ft.Colors.BLUE_100),
                initially_expanded=True,  # <- Hace que esté desplegado al cargar
                controls=[
                    ft.Container(
                        content=table,
                        padding=10
        ...
        # Usamos ft.ListView para hacer que la página sea desplazable
        return ft.ListView(
            controls=cards,
            height=600,
        )
...
def main(page):
    ...
    # Crear el dashboard
    dashboard = CompanyDashboard(overview, income, margins, balance, ratios)
    page.controls.append(dashboard.render())

    # Actualizar la página para renderizar los controles
    page.update()
