...
def _fmt(n: int | float) -> str:
    # Numbers as strings without symbols/commas (safe_int parses them)
    return str(int(n)) if isinstance(n, int) or float(n).is_integer() else f"{n:.2f}"

def seed_symbol(symbol: str = "MSFT"):
    """
    Inserts consistent DEMO data into multiple collections and also
    creates a 'canonical' document in company_profiles with the shape expected by
    DashboardData / CompanyData.get_full_profile():
      {
        symbol: "MSFT",
        overview: {...},
        logo: "https://...",
    ...
    # --- Base overview (also reused for the canonical profile) ---
    overview_doc = {
        "Symbol": symbol,
        "Ticket": symbol,
        "Name": "Microsoft Corporation (DEMO)",
        "Description": "Provider of software, services, and devices. Demonstration data.",
        "Exchange": "NASDAQ",
        "Currency": "USD",
        "Country": "USA",
        "Sector": "Technology",
    ...
    # --- Analysis (for RAG block) ---
    db["analysis"].update_one(
        {"symbol": symbol},
        {"$set": {
            "summary": (
                "DEMO: Strong revenue and margins with sustained operational expansion. "
                "Cash position enables continued investment in AI and cloud; leverage decreasing."
            )
        }},
        upsert=True,
    )
