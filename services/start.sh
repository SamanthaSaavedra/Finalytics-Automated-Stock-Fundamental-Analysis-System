...
export PYTHONUNBUFFERED=1  # Ensure Python prints appear immediately

wait_for_main_api() {
    echo "Waiting for main_api to finish loading..."
    for i in {1..15}; do  # up to ~5 minutes (15*20s)
        sleep 20
        STATUS=$(curl -sf http://127.0.0.1:8100/health | grep -o '"status": *"ok"' || echo "")
...
exec uvicorn services.main_api:app --host 0.0.0.0 --port 8100 &
UVICORN_PID=$!
