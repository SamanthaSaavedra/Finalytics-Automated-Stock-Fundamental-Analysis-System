...
if not SEC_API_KEYS:
    raise RuntimeError("SEC_API_KEYS env var está vacía o mal formateada")
...
class CompanyRAGDocument:
    ...
    def __init__(self, symbol: str, *, verbose: bool = False):
        ...
        self.url = f"http://controller:8100/api/dashboard/{self.symbol}"
        self.document = f"http://controller:8100/document/"
        ...
        existing_doc_resp = requests.get(f"{self.document}/{self.symbol}")

        self.url = f"http://controller:8100/api/dashboard/{self.symbol}"
        self.document_url = f"http://controller:8100/document/{self.symbol}"

        try:
            existing_doc_resp = requests.get(self.document_url)
            print("Status code:", existing_doc_resp.status_code)

            try:
                data = existing_doc_resp.json()
                print("Response JSON:", data)
            except ValueError:
                print("No se recibió JSON válido desde el servidor.")
                data = None

            
            if existing_doc_resp.status_code == 200 and data and data.get("exists") is True:
                print(f"The document for {self.symbol} already exists. It will not be saved again.")

                doc = data.get("document", {})
                self.trend_summary = doc.get("trend_summary", "")
                self.table_summary = doc.get("table_summary", "")
                self.sec_text_full_clean = doc.get("sec_text_full_clean", "")
                return

            else:
                print(f"No existing document found for {self.symbol}. Generating new one...")

        except requests.RequestException as e:
            print(f"Error connecting to document service: {e}")
            data = None
    ...
    def load_dashboard_data(self):
        try:
            ...
            if resp.status_code != 200:
                raise ValueError(f"Error fetching data from API: {resp.status_code}")
            data = resp.json()
            ...
            raise ValueError(f"Error fetching dashboard data: {e}")
    ...
    def get_filing_url_by_ticker(ticker, form_type="10-K"):
        ...
        headers = {"User-Agent": "proyectname/example@gmail.com"}
        ...
        raise ValueError(f"No se encontró un {form_type} para {ticker}")
    ...
    def save_to_mongo(self):
        ...
        if existing_doc_resp.status_code == 200 and existing_doc_resp.json().get("exists"):
            print(f"The document for {self.symbol} already exists. It will not be saved again.")
            return
        ...
        response = requests.post(self.document, json=data_to_save)

        if response.status_code == 200:
            print(f"Document saved in MongoDB for {self.symbol}")
        else:
            print(f"Failed to save document for {self.symbol}. Status code: {response.status_code}")
            try:
